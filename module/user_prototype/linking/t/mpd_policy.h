...
static void mpd_edge_init(struct edge *e)
{
...
static void pd_edge_init(struct pd_edge *e)
{
...
static void mpd_component_init(struct component *c, spdid_t id)
{
...
static struct component *mpd_component_alloc(spdid_t spdid, struct protection_domain *pd)
{
	struct component *c;
	...
	if (NULL == pd->members) pd->members = c;
	else ADD_LIST(pd->members, c, pd_next, pd_prev);
	pd->nmembs++;
	c->pd = pd;
...
static void mpd_component_free(struct component *c)
{
...
static struct protection_domain *pd_create(void)
{
	struct protection_domain *pd;
...
static inline long pd_edge_get_weight(struct pd_edge *pde)
{
	assert(pde);
...
static inline void pd_edge_set_weight(struct pd_edge *pde, long val)
{
	assert(pde);
...
static inline long edge_get_inv(struct edge *e)
{
	return e->invocations;
}

static inline void edge_set_inv(struct edge *e, long val)
{
...
static void pd_edge_free(struct pd_edge *pd_e)
{
...
static struct pd_edge *pd_edge_create(struct protection_domain *pd_from, 
				      struct protection_domain *pd_to)
{
	struct pd_edge *pd_e;
...
static void pd_free(struct heaps *hs, struct protection_domain *pd)
{
	struct component *t;
	...
	if (!EMPTY_LIST(&pd->inward_edges, from_next, from_prev)) {
		struct pd_edge *e;
		for (e = FIRST_LIST(&pd->inward_edges, from_next, from_prev) ; 
		     e != &pd->inward_edges ;
 ...
 * are "from" or "to".
 */
static struct pd_edge *__pd_find_edge(struct protection_domain *pd1, 
				      struct protection_domain *pd2, int create)
{
	...
	for (e = FIRST_LIST(&pd1->outward_edges, to_next, to_prev) ;
	     e != &pd1->outward_edges ; 
	     e = FIRST_LIST(e, to_next, to_prev)) {
		if ((e->from == pd1 && e->to == pd2) ||
		    (e->from == pd2 && e->to == pd1)) return e;
	}
	for (e = FIRST_LIST(&pd1->inward_edges, from_next, from_prev) ;
	     e != &pd1->inward_edges ; 
	     e = FIRST_LIST(e, from_next, from_prev)) {
		if ((e->from == pd1 && e->to == pd2) ||
		    (e->from == pd2 && e->to == pd1)) return e;
...
static struct pd_edge *pd_find_edge(struct protection_domain *pd1, 
				    struct protection_domain *pd2)
{
	return __pd_find_edge(pd1, pd2, 1);
}

static int mpd_component_add_edge(struct component *from, struct component *to, 
				  long weight)
{
	struct edge *e;
	struct pd_edge *pd_e;
...
static inline void pd_remove_pd_edge(struct edge *e)
{
	struct pd_edge *pd_e;
 ...
 */
static int pd_rem_component(struct heaps *hs, struct protection_domain *pd, struct component *c)
{
	struct edge *e;
	...
	for (e = FIRST_LIST(&c->outward_edges, to_next, to_prev) ;
	     e != &c->outward_edges ;
	     e = FIRST_LIST(e, to_next, to_prev)) {
		pd_remove_pd_edge(e);
	}
	for (e = FIRST_LIST(&c->inward_edges, from_next, from_prev) ;
	     e != &c->inward_edges ;
	     e = FIRST_LIST(e, from_next, from_prev)) {
		pd_remove_pd_edge(e);
	}
...
static inline void pd_add_edge(struct edge *e, struct protection_domain *from, 
			       struct protection_domain *to)
{
	struct pd_edge *pd_e;
...
static void pd_add_component(struct protection_domain *pd, struct component *c)
{
	struct edge *e;
...
static void __pd_merge(struct protection_domain *pd, struct protection_domain *pd_fin, struct heaps *hs)
{
...
static void pd_merge(struct pd_edge *pd_e, struct heaps *hs) 
{
 ...
 */
static struct protection_domain *pd_split(struct protection_domain *pd, struct heaps *hs)
{
...
static inline int pd_edge_internal(struct edge *e)
{
...
static long component_grp_weight(struct component *s, struct component *grp)
{
	...
	for (e = FIRST_LIST(&s->outward_edges, to_next, to_prev) ; 
	     e != &s->outward_edges ; 
	     e = FIRST_LIST(e, to_next, to_prev)) {
		assert(e->to);
		if (pd_edge_internal(e) && 
		    e->to->sg_master == master) cnt += edge_get_inv(e);
	}
	for (e = FIRST_LIST(&s->inward_edges, from_next, from_prev) ; 
	     e != &s->inward_edges ; 
	     e = FIRST_LIST(e, from_next, from_prev)) {
		assert(e->from);
		if (pd_edge_internal(e) && 
		    e->from->sg_master == master) cnt += edge_get_inv(e);
	}
...
static void __component_heap_adjust(struct component *c, struct edge *e, struct heap *h)
{
...
static void component_adjust_heap(struct component *s, struct heap *h)
{
	struct edge *e;
...
static void component_grp_adjust_heap(struct component *g, struct heap *h)
{
...
static inline long mc_calculate_value(struct component *mc, 
				      struct protection_domain *pd)
{
 ...
 */
static long mc_phase(struct heap *h, struct protection_domain *pd, 
		     struct component **second_to_last, 
		     struct component **mcop)
{
	...
	for (t = FIRST_LIST(s, pd_next, pd_prev) ; 
	     t != s ; 
	     t = FIRST_LIST(t, pd_next, pd_prev)) {
		assert(t != s);
		if (t->sg_master == t) {
...
static void mc_reset_cop(struct protection_domain *pd)
{
	struct component *t;
...
static void mc_append_lists(struct component *a, struct component *b)
{
	struct component *t;
...
static void mc_prepare_data(struct protection_domain *pd)
{
	struct component *t;
 ...
 */
static struct component *mc_find_min_cut(struct protection_domain *pd, struct heap *h)
{
...
static void try_create_component(spdid_t id)
{
...
static void create_components(struct comp_graph *ies)
{
	int i;
...
static int mc_cmp(void *a, void *b)
{
...
static void pd_update(void *a, int pos)
{
...
static int pde_cmp(void *a, void *b)
{
...
static void pde_update(void *a, int pos)
{
	struct pd_edge *e = a;
...
static void mpd_increase_isolation(struct heaps *hs)
{
	struct protection_domain *pd;
...
static long mpd_peek_inc_isolation(struct heaps *hs)
{
	assert(hs && hs->pd_h);
	assert(heap_size(hs->pd_h) > 0);
...
static long mpd_peek_dec_overhead(struct heaps *hs)
{
	assert(hs && hs->pde_h);
	assert(heap_size(hs->pde_h) > 0);
...
static int mpd_empty_inc_isolation(struct heaps *hs)
{
	assert(hs && hs->pd_h);
	return heap_empty(hs->pd_h);
}

static int mpd_empty_dec_overhead(struct heaps *hs)
{
	assert(hs && hs->pde_h);
	return heap_empty(hs->pde_h);
}

static void recompute_min_cuts(struct protection_domain *pds, struct heaps *hs)
{
	struct protection_domain *pd;
...
static void test_edge_weight_consistency(void)
{
	struct edge *e;
	struct pd_edge *pd_e;
	
	for (pd_e = FIRST_LIST(&pdes, next, prev) ; 
	     pd_e != &pdes ; 
	     pd_e = FIRST_LIST(pd_e, next, prev)) {
		int total = 0;
		for (e = FIRST_LIST(&pd_e->constituent_edges, pd_next, pd_prev) ; 
		     e != &pd_e->constituent_edges ; 
...
/* 	return; */
	...
	test_edge_weight_consistency();
	while (!mpd_empty_inc_isolation(&hs) && 
	       tot_cost + mpd_peek_inc_isolation(&hs) < allowed_invs) {
		mpd_increase_isolation(&hs);
	}
	test_edge_weight_consistency();
...
static void merge_all_components(struct component *cs, struct heaps *hs)
{
...
static void split_all_components(struct protection_domain *pds, struct heaps *hs)
{
	struct protection_domain *first;
	/* only one pd allowed! */
	assert(FIRST_LIST(pds, next, prev) == LAST_LIST(pds, next, prev) &&
	       !EMPTY_LIST(pds, next, prev));
...
//#define MAX_COMPONENTS 14
static struct comp_graph ies[MAX_COMPONENTS+1] = 
{
	{.client = 1, .server = 2},
	{.client = 2, .server = 3},
	{.client = 3, .server = 4},
...
static void edges_reset_random_weights(void)
{
	struct edge *e;
...
static void merge_split_repeat(struct heaps *hs)
{
...
static void test_repeat(struct heaps *hs)
{
	int i;
