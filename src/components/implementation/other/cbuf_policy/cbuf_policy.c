...
gather_data(int counter)
{
	...
	for (titer = FIRST_LIST(&threads, next, prev) ;
		...
		for (citer = FIRST_LIST(&components, next, prev) ;
			...
			tc->stack_misses = cbufmgr_thd_blk_cnt(tid, tc->c->spdid, 0);
			...
			if (counter == 0 && tc->stack_misses) {
			 	printc("\tCbuf info for %d: time blocked %ld, misses %d\n",
...
set_concur_new(void)
{
	...
	for (c = FIRST_LIST(&components, next, prev) ;
		...
		/* i = c->spdid; */
		/* if (i==spd_left) cbufmgr_set_concurrency(c->spdid, t_right, 1); */
		/* if (i==spd_right) cbufmgr_set_concurrency(c->spdid, t_left, 1); */
	}
...
/* Allocate at least ss_counter + 1 cbufs to self-suspension
...
solve_suspension(void)
{
	...
	for (citer = FIRST_LIST(&components, next, prev) ;
		...
		if (citer->concur_new < citer->ss_counter + 1) {
			...
			printc("allocate cbufs to comp %d for suspension, cnt %d. concur_new %d -> %d \n",
...
init_thds(void)
{
	...
	for (i = 0 ; i < MAX_NUM_THREADS ; i++) {
		...
		printc("CBUF Policy: Found thread %d.\n", i);
...
/* 		else{		 */
/* 			i = c->spdid; */
/* 			if (i==spd_left) cbufmgr_set_concurrency(c->spdid, t_left, 0); */
/* 			if (i==spd_right) cbufmgr_set_concurrency(c->spdid, t_right, 0); */
/* 			if (i!=spd_left && i!=spd_right) cbufmgr_set_concurrency(c->spdid, t_init, 0);  */
/* 		} */
/* 	} */
/* 	//stkmgr_set_over_quota_limit(available); */
/* } */
...
#ifdef THD_POOL
	printc("<<<Thd Pool with total %d cbufs, component size %d>>>\n", MAX_NUM_CBUFS, THD_POOL);
	if (THD_POOL != 1)
		thdpool_max_policy();
	else
		thdpool_1_policy();
#else
	printc("<<<Now using Algorithm %d, total number of cbufs:%d >>>\n", ALGORITHM, MAX_NUM_CBUFS);
	init_policy();
#endif

	printc("CBUFPol: %d in spd %ld other thds are not running..\n", cos_get_thd_id(), cos_spd_id());
