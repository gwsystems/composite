...
comps_init(void)
{
	...
	for (cont = args_iter(&comps, &i, &curr) ; cont ; cont = args_iter_next(&i, &curr)) {
		...
		if (id == cos_compid()) {
			...
			cos_ulk_info_init(cos_compinfo_get(comp->comp_res));
			cos_ulk_map_in(cos_compinfo_get(comp->comp_res)->pgtbl_cap);
			assert(ret == 0);
		} else {
			assert(elf_hdr);
...
chkpt_comp_init(struct crt_comp *comp, struct crt_chkpt *chkpt, char *name)
{
	...
	if (id == cos_compid()) {
		/* this should never crt_ns_vas_ulk_maphappen */
		assert(0);
	} else {
