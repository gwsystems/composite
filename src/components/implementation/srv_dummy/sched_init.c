...
schedinit_child(void)
{
	...
	tcur = sl_thd_curr();
	if (!tcur) return 0;
	if (!(ci->flags & COMP_FLAG_SCHED)) {
		assert(tcur == init); /* initthd must call this if it's not a scheduler */
		goto done;
	}
	...
	/* This thread will only be able to make "functional" invocations after this point. */
	srv_dummy_thdinit(sl_thd_thdid(init), ci->flags & COMP_FLAG_SCHED);
