...
ping_test_objread(void)
{
	...
	reqtok = pongshmem_test_map(id);

	for (i = 0; i < 4; i++) {
		/* allocate an object from shared mem buffer */
		obj = (struct obj_test *) shm_bm_clt_alloc_testobj(shm, &objid);
		...
		pongshmem_test_objread(reqtok, objid, i);
...
ping_test_bigalloc(void)
{
	...
	id = shm_bm_clt_create_testobj(&shm);
	reqtok = pongshmem_bench_map(id);
	for (i = 0; i < BENCH_ITER; i++) {
		if ((obj = shm_bm_clt_alloc_testobj(shm, &objid)) == 0) {
			failure = 1;
			goto bigallocdone;
		}
		obj->id = i;
	...
	for (i = 0; i < BENCH_ITER; i++) {
		pongshmem_bench_objread(reqtok, (shm_objid_t) i); 	
	}

bigallocdone:
	PRINTLOG(PRINT_DEBUG, "%s: Ping can allocate from big shared buffer\n", (failure) ? "FAILURE" : "SUCCESS");
	
	failure = shm_bm_clt_alloc_testobj(shm, &objid) != 0;
...
ping_test_objfree(void)
{
	...
	for (i = 0; i < BENCH_ITER-2; i++) {
		if (shm_bm_clt_alloc_testobj(shm, &objid) == 0)
			failure = 1;
	}
...
ping_test_bigfree(void)
{
	...
	for (i = 0; i < BENCH_ITER; i++) {
		if ((obj_ptrs[i] = shm_bm_clt_alloc_testobj(shm, &objid)) == 0) {
			failure = 1;
			goto bigallocdone;
		}
		obj_ptrs[i]->id = i;
...
bigallocdone:
	PRINTLOG(PRINT_DEBUG, "%s: Ping can allocate and reallocate from big shared buffer\n", (failure) ? "FAILURE" : "SUCCESS");
...
ping_test_refcnt(void)
{
	...
	obj = shm_bm_clt_alloc_testobj(shm, &objid); 
	 ...
	 */ 
	pongshmem_test_refcnt(reqtok, objid);
...
ping_bench_msgpassing_static(void)
{
	...
	for (i = 0; i < BENCH_ITER; i++) {
		...
		/* send obj to server, server borrows it */
		obj->id = objid;
		pongshmem_bench_objread(reqtok, objid);
	}
	end = ps_tsc();
	bench = (end - begin) / BENCH_ITER;
	PRINTLOG(PRINT_DEBUG, "BENCHMARK Message passing (static): %llu cycles\n", bench);
}
