...
test_ipi_interference(void)
{
	...
	if (cos_cpuid() == TEST_RCV_CORE) {
		...
		if (EXPECT_LL_LT(1, tcc, "IPI Interference: TCAP Allocation")) return;

		t = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_rcv_fn, NULL);
		if (EXPECT_LL_LT(1, t, "IPI Inteference: Thread Allocation")) return;

		r = cos_arcv_alloc(&booter_info, t, tcc, booter_info.comp_cap, BOOT_CAPTBL_SELF_INITRCV_CPU_BASE);
		if (EXPECT_LL_LT(1, r, "IPI Interference: ARCV Allocation")) return;
		...
		if (EXPECT_LL_LT(1, t, "IPI Interference: Thread Allocation")) return;

		spinner_thd[cos_cpuid()] = t;

		s = cos_asnd_alloc(&booter_info, rcv[TEST_SND_CORE], booter_info.captbl_cap);
		if (EXPECT_LL_LT(1, s, "IPI Interference: ASND Allocation")) return;
