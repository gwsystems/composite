...
{
	while (
	  cos_sched_rcv(BOOT_CAPTBL_SELF_INITRCV_CPU_BASE, RCV_ALL_PENDING, 0, rcvd, tid, blocked, cycles, thd_timeout)
	  != 0)
		;
}
...
rcv_spinner(void *d)
{
	while (!done_test) { rdtscll(global_time[0]); }
...
test_asnd_fn(void *d)
{
	...
	PRINTC("Test IPI INTERRUPT W Switch:\t AVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", perfdata_avg(&pd),
	       perfdata_max(&pd), perfdata_min(&pd), perfdata_sz(&pd));
...
test_sched_loop(void)
{
	...
	while (1) {
		if (cos_cpuid() == TEST_RCV_CORE) {
			do {
				ret = cos_switch(spinner_thd[cos_cpuid()], BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE,
				                 TCAP_PRIO_MAX + 2, 0, 0, 0);
			} while (ret == -EAGAIN);
		}
...
test_ipi_switch(void)
{
	...
	if (cos_cpuid() == TEST_RCV_CORE) {
		...
		if (EXPECT_LL_LT(1, tcc, "IPI SWITCH: TCAP Allocation")) return;

		t = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_rcv_fn, NULL);
		if (EXPECT_LL_LT(1, t, "IPI SWITCH: Thread Allocation")) return;

		r = cos_arcv_alloc(&booter_info, t, tcc, booter_info.comp_cap, BOOT_CAPTBL_SELF_INITRCV_CPU_BASE);
		if (EXPECT_LL_LT(1, r, "IPI SWITCH: ARCV Allocation")) return;
		...
		if (EXPECT_LL_LT(1, s, "IPI SWITCH: ASND Allocation")) return;
		asnd[cos_cpuid()] = s;
		test_sync_asnd();
		test_sched_loop();
