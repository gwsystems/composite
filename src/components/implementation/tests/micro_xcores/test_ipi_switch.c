...
test_ipi_switch(void)
{
        ...
        if (cos_cpuid() == TEST_RCV_CORE) {
                ...
                if (EXPECT_LL_LT(1, tcc, "IPI SWITCH: TCAP Allocation"))
                        return;
                t = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_rcv_fn, NULL, 0, 0, 0, 0);
                ...
                t = cos_thd_alloc(&booter_info, booter_info.comp_cap, rcv_spinner, NULL, 0, 0, 0, 0);
                if (EXPECT_LL_LT(1, t, "IPI SWITCH: Thread Allocation"))
                        return;
                ...
                t = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_asnd_fn, NULL, 0, 0, 0, 0);
                if (EXPECT_LL_LT(1, t, "IPI SWITCH: Thread Allocation"))
                        return;
