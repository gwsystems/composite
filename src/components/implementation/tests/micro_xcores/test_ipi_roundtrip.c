...
test_rcv(arcvcap_t r)
{
	...
	assert(pending == 0);
	if (EXPECT_LL_LT(1, r, "IPI Roundtrip: Allocation on RCV")) cos_thd_switch(BOOT_CAPTBL_SELF_INITTHD_CPU_BASE);
...
test_sched_loop(void)
{
	...
	while (1) {
		while ((pending = cos_sched_rcv(BOOT_CAPTBL_SELF_INITRCV_CPU_BASE, RCV_ALL_PENDING, 0, &rcvd, &thdid,
...
test_asnd_fn(void *d)
{
	...
	while (1) {
		...
		iters++;
		if (iters >= TEST_IPI_ITERS) { break; }
	}

	perfdata_calc(&pt[cos_cpuid()]);
	PRINTC("Test IPI Send:\t\t\t SEND TIME AVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", perfdata_avg(&pt[cos_cpuid()]),
	       perfdata_max(&pt[cos_cpuid()]), perfdata_min(&pt[cos_cpuid()]), perfdata_sz(&pt[cos_cpuid()]));
	printc("\t\t\t\t\t SD:%llu, 90%%:%llu, 95%%:%llu, 99%%:%llu\n", perfdata_sd(&pt[cos_cpuid()]),
	       perfdata_90ptile(&pt[cos_cpuid()]), perfdata_95ptile(&pt[cos_cpuid()]),
	       perfdata_99ptile(&pt[cos_cpuid()]));
...
test_ipi_roundtrip(void)
{
	...
	if (cos_cpuid() == TEST_RCV_CORE) {
		...
		if (EXPECT_LL_LT(1, tcc, "IPI ROUNDTRIP: TCAP Allocation")) return;


		t = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_rcv_fn, NULL);
		if (EXPECT_LL_LT(1, t, "IPI ROUNDTRIP: Thread Allocation")) return;

		r = cos_arcv_alloc(&booter_info, t, tcc, booter_info.comp_cap, BOOT_CAPTBL_SELF_INITRCV_CPU_BASE);
		if (EXPECT_LL_LT(1, r, "IPI ROUNDTRIP: ARCV Allocation")) return;
		...
		if (EXPECT_LL_LT(1, s, "IPI ROUNDTRIP: ASND Allocation")) return;
		...
		r = cos_arcv_alloc(&booter_info, t, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, booter_info.comp_cap,
