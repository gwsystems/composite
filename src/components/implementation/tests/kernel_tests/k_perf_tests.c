...
test_thds_create_switch(void)
{
	...
	if (EXPECT_LL_LT(1, ts, "Thread Creation: Cannot Allocate")) { return; }
	...
	printc("\t\t\t\t\t\t\tSD:%llu, 90%%:%llu, 95%%:%llu, 99%%:%llu\n", perfdata_sd(&pd[cos_cpuid()]),
	       perfdata_90ptile(&pd[cos_cpuid()]), perfdata_95ptile(&pd[cos_cpuid()]),
	...
	PRINTC("\tCOS THD => COS_SWITCH:\t\t\tAVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", perfdata_avg(&pd[cos_cpuid()]),
	       perfdata_max(&pd[cos_cpuid()]), perfdata_min(&pd[cos_cpuid()]), perfdata_sz(&pd[cos_cpuid()]));
...
test_async_endpoints_perf(void)
{
	...
	if (EXPECT_LL_NEQ(0,
	                  cos_tcap_transfer(rcp, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, TCAP_RES_INF, TCAP_PRIO_MAX + 1),
	...
	if (EXPECT_LL_NEQ(0, cos_tcap_transfer(rcc, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, TCAP_RES_INF, TCAP_PRIO_MAX),
	                  "Test Async Endpoints"))
		return;
...
{
	PRINTC("\tSINV:\t\t\t\t\tAVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", result_sinv.avg, result_sinv.max,
	       result_sinv.max, result_sinv.sz);

	printc("\t\t\t\t\t\t\tSD:%llu, 90%%:%llu, 95%%:%llu, 99%%:%llu\n", result_sinv.sd, result_sinv.p90tile,
	       result_sinv.p95tile, result_sinv.p99tile);

	PRINTC("\tTimer => Timeout Overhead: \t\tAVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", result_test_timer.avg,
	       result_test_timer.max, result_test_timer.min, result_test_timer.sz);

	printc("\t\t\t\t\t\t\tSD:%llu, 90%%:%llu, 95%%:%llu, 99%%:%llu\n", result_test_timer.sd,
	       result_test_timer.p90tile, result_test_timer.p95tile, result_test_timer.p99tile);

	PRINTC("\tTimer => Budget based: \t\t\tAVG:%llu, MAX:%llu, MIN:%llu, ITER:%d\n", result_budgets_single.avg,
	       result_budgets_single.max, result_budgets_single.min, result_budgets_single.sz);

	printc("\t\t\t\t\t\t\tSD:%llu, 90%%:%llu, 95%%:%llu, 99%%:%llu\n", result_budgets_single.sd,
	       result_budgets_single.p90tile, result_budgets_single.p95tile, result_budgets_single.p99tile);
}
