...
test_thds_create_switch(void)
{
        ...
        ts = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_thd_arg, (void *)i, 0, 0, 0, 0);
...
test_mthds_ring(void)
{
        ...
        for (i = 0; i < TEST_NTHDS; i++) {
                thd_test[i] = cos_thd_alloc(&booter_info, booter_info.comp_cap, thd_fn_mthds_ring, (void *)i, 0, 0, 0, 0);
...
test_mthds_classic(void)
{
        ...
        ts = cos_thd_alloc(&booter_info, booter_info.comp_cap, thd_fn_mthds_classic, NULL, 0, 0, 0, 0);
...
test_thds_tls(void)
{
        ...
        for (i = 0; i < TEST_NTHDS; i++) {
                ts[i] = cos_thd_alloc(&booter_info, booter_info.comp_cap, thd_tls, (void *)i, 0, 0, 0, 0);
...
test_thds_fpu(void)
{
        ...
        thds[0] = cos_thd_alloc(&booter_info, booter_info.comp_cap, test_thds_reg, (void *)i, 0, 0, 0, 0);
        for (i = 1; i <= 3; i++) {
                thds[i] = cos_thd_alloc(&booter_info, booter_info.comp_cap, thds_fpu, (void *)i, 0, 0, 0, 0);
