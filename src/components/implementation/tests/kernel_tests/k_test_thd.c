...
test_mthds_ring(void)
{
	...
	for (i = 0; i < TEST_NTHDS; i++) {
		...
		if (EXPECT_LL_LT(1, thd_test[i], "Thread Ring: Cannot Allocate")) { return; }
	}

	ret = cos_thd_switch(thd_test[0]);
	EXPECT_LL_NEQ(0, ret, "Thread Ring: COS Switch Error");

	if (EXPECT_LL_NEQ(count, TEST_NTHDS, "Thread Ring: Failure # of THDS")) { return; }
...
test_mthds_classic(void)
{
	...
	if (EXPECT_LL_LT(1, ts, "Thread Classic: Cannot Allocate")) { return; }
...
test_thds_tls(void)
{
	...
	for (i = 0; i < TEST_NTHDS; i++) {
		...
		if (EXPECT_LL_LT(1, ts[i], "Thread TLS: Cannot Allocate")) { return; }
