...
async_thd_fn(void *thdcap)
{
	...
	if (EXPECT_LL_NEQ(0, ret, "COS Switch Error") || EXPECT_LL_NEQ(0, 1, "Test Async Endpoints")) { failure = 1; }
...
test_async_endpoints(void)
{
	...
	if (EXPECT_LL_NEQ(0,
	                  cos_tcap_transfer(rcc, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, TCAP_RES_INF, TCAP_PRIO_MAX + 1),
	                  "Test Async Endpoints")) {
		return;
	}
