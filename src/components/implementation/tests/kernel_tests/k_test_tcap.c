...
sched_events_clear(void)
{
	...
	while (cos_sched_rcv(BOOT_CAPTBL_SELF_INITRCV_CPU_BASE, RCV_ALL_PENDING, 0, &rcvd, &tid, &blocked, &cycles,
	                     &thd_timeout)
	       != 0)
		;
}
...
test_timer(void)
{
	...
	cos_sched_rcv(BOOT_CAPTBL_SELF_INITRCV_CPU_BASE, RCV_ALL_PENDING, 0, &rcvd, &tid, &blocked, &cycles,
	              &thd_timeout);
...
test_2timers(void)
{
	...
	if (EXPECT_LL_NEQ(0,
	                  exec_cluster_alloc(&bt[cos_cpuid()].p, parent, &bt[cos_cpuid()].p,
	                                     BOOT_CAPTBL_SELF_INITRCV_CPU_BASE),
	                  "TCAP v. Timer: Cannot Allocate")) {
		return;
	}
	if (EXPECT_LL_NEQ(0, exec_cluster_alloc(&bt[cos_cpuid()].c, spinner, &bt[cos_cpuid()].c, bt[cos_cpuid()].p.rc),
	                  "TCAP v. Timer: Cannot Allocate")) {
		return;
	}
	...
	ret = cos_tcap_transfer(bt[cos_cpuid()].c.rc, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, GRANULARITY * TIMER_TIME,
	                        TCAP_PRIO_MAX + 2);
	if (EXPECT_LL_NEQ(0, ret, "TCAP v. Timer : TCAP Transfer")) { return; }

	rdtscll(s);
	timer = tcap_cyc2time(s + GRANULARITY * cyc_per_usec);
	if (cos_switch(bt[cos_cpuid()].c.tc, bt[cos_cpuid()].c.tcc, TCAP_PRIO_MAX + 2, timer,
	...
	ret = cos_tcap_transfer(bt[cos_cpuid()].c.rc, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, GRANULARITY * cyc_per_usec,
	                        TCAP_PRIO_MAX + 2);
	if (EXPECT_LL_NEQ(0, ret, "TCAP v. Timer: TCAP Transfer")) { return; }
	...
	if (EXPECT_LL_NEQ(0,
	                  cos_switch(bt[cos_cpuid()].c.tc, bt[cos_cpuid()].c.tcc, TCAP_PRIO_MAX + 2, timer,
	                             BOOT_CAPTBL_SELF_INITRCV_CPU_BASE, cos_sched_sync()),
	                  "TCAP v. TImer: COS Switch")) {
		return;
	}
...
test_tcap_budgets_single(void)
{
	...
	for (i = 1; i <= TEST_ITER; i++) {
		...
		if (EXPECT_LL_NEQ(0, ret, "Single Budget: TCAP Transfer")) { return; }
		...
		if (i > 1) {
			...
			if (EXPECT_LLU_LT((long long unsigned)(e - s), (unsigned)(GRANULARITY * BUDGET_TIME * MAX_THDS),
			    ...
			    || EXPECT_LLU_LT((unsigned)(GRANULARITY * BUDGET_TIME * MIN_THDS),
			                     (long long unsigned)(e - s), "Single Budget: MIN Bound")) {
				return;
			}
		}
		sched_events_clear();
...
test_tcap_budgets_multi(void)
{
	...
	if (EXPECT_LL_NEQ(0,
	                  exec_cluster_alloc(&mbt[cos_cpuid()].p, spinner_cyc, &(mbt[cos_cpuid()].p.cyc),
	                                     BOOT_CAPTBL_SELF_INITRCV_CPU_BASE),
	                  "Multi Budget: Cannot Allocate")
	    || EXPECT_LL_NEQ(0,
	                     exec_cluster_alloc(&mbt[cos_cpuid()].c, spinner_cyc, &(mbt[cos_cpuid()].c.cyc),
	                                        mbt[cos_cpuid()].p.rc),
	                     "Multi Budget: Cannot Allocate")
	    || EXPECT_LL_NEQ(0,
	                     exec_cluster_alloc(&mbt[cos_cpuid()].g, spinner_cyc, &(mbt[cos_cpuid()].g.cyc),
	                                        mbt[cos_cpuid()].c.rc),
	                     "Multi Budget: Cannot allocate")) {
		return;
	}
	...
	for (i = 1; i <= TEST_ITER; i++) {
		...
		if (EXPECT_LL_NEQ(0,
		                  cos_tcap_transfer(mbt[cos_cpuid()].p.rc, BOOT_CAPTBL_SELF_INITTCAP_CPU_BASE, res,
		                                    TCAP_PRIO_MAX + 2),
		                  "Multi Budget: TCAP Transfer")
		    || EXPECT_LL_NEQ(0,
		                     cos_tcap_transfer(mbt[cos_cpuid()].c.rc, mbt[cos_cpuid()].p.tcc, res / 2,
		                                       TCAP_PRIO_MAX + 2),
		                     "Multi Budget: TCAP Transfer")
		    || EXPECT_LL_NEQ(0,
		                     cos_tcap_transfer(mbt[cos_cpuid()].g.rc, mbt[cos_cpuid()].c.tcc, res / 4,
		                                       TCAP_PRIO_MAX + 2),
		                     "Multi Budget: TCAP Transfer")) {
			return;
		}
		...
		if (i > 1) {
			...
			if (EXPECT_LLU_LT((mbt[cos_cpuid()].g.cyc - s), (res / 4 * MAX_THDS), "Multi Budget: G")
			    || EXPECT_LLU_LT(mbt[cos_cpuid()].g.cyc - s, res / 4 * MAX_THDS,
			                     "Multi Budget: G MAX Bound")
			    || EXPECT_LLU_LT(res / 4 * MIN_THDS, mbt[cos_cpuid()].g.cyc - s,
			                     "Multi Budget: G MIN Bound")
			    || EXPECT_LLU_LT(mbt[cos_cpuid()].c.cyc - s, res / 2 * MAX_THDS,
			                     "Multi Budget: C MAX Bound")
			    || EXPECT_LLU_LT(res / 2 * MIN_THDS, mbt[cos_cpuid()].c.cyc - s,
			                     "Multi Budget: C MIN Bound")
