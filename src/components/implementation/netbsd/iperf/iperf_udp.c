...
iperf_udp_recv(struct iperf_stream *sp)
{
    ...
    if (pcount >= (unsigned) sp->packet_count + 1) {
        if (pcount > (unsigned) sp->packet_count + 1) {
	...
	iperf_err(sp->test, "OUT OF ORDER - incoming packet = %zu and received packet = %d AND SP = %d", (size_t) pcount, sp->packet_count, sp->socket);
    }
