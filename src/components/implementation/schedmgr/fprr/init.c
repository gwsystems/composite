...
cos_init(void)
{
	...
	printc("%lu CPU cycles per sec: %u\n", cos_spd_id(), cos_hw_cycles_per_usec(BOOT_CAPTBL_SELF_INITHW_BASE));
	...
	for (i = 0; i < MAX_CHILD_BITS; i++) {
		if (IS_BIT_SET(child_spdbits, i)) {
			printc("%lu: Initializing child component %d\n", cos_spd_id(), i + 1);
	...
	printc("%lu ERROR: Should never have reached this point!!!\n", cos_spd_id());
	assert(0);
}
