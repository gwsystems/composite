...
#define SL_FP_NPRIOS         32
#define SL_FP_PRIO_HIGHEST   TCAP_PRIO_MAX
#define SL_FP_PRIO_LOWEST    SL_FP_NPRIOS

#define SL_FP_PERIOD_US_MIN  SL_MIN_PERIOD_US

struct ps_list_head threads[NUM_CPU][SL_FP_NPRIOS] CACHE_ALIGNED;
...
sl_mod_thd_param_set(struct sl_thd_policy *t, sched_param_type_t type, unsigned int v)
{
	...
	case SCHEDP_PRIO:
	{
		assert(v >= SL_FP_PRIO_HIGHEST && v <= SL_FP_PRIO_LOWEST);
...
sl_mod_init(void)
{
	...
	memset(threads[cos_cpuid()], 0, sizeof(struct ps_list_head) * SL_FP_NPRIOS);
	for (i = 0 ; i < SL_FP_NPRIOS ; i++) {
