...
#define SHM_BM_BITMAP_BLOCK (sizeof(uword_t) * CHAR_BIT)
...
#define SHM_BM_REFC(shm, nobj) ((unsigned char *)((unsigned char *)shm + SHM_BM_BITS_TO_WORDS(nobj) * sizeof (uword_t)))
#define SHM_BM_DATA(shm, nobj) ((unsigned char *)(SHM_BM_REFC(shm, nobj) + (unsigned int)nobj))

static inline void
__shm_bm_set_contig(uword_t *bm, int offset)
{
	int i, n, ind;
...
static inline int
__shm_bm_next_free_word(uword_t *bm, unsigned long nwords)
{
...
__shm_bm_size(size_t objsz, unsigned int nobj)
{
	...
	bitmap_sz = SHM_BM_BITS_TO_WORDS(nobj) * sizeof(uword_t);
	refcnt_sz = nobj;
	data_sz   = nobj * objsz;
...
{
	if ((uword_t)mem % SHM_BM_ALIGN != 0) return 0;
...
__shm_bm_init(shm_bm_t shm, size_t objsz, unsigned int nobj)
{
	...
	__shm_bm_set_contig((uword_t *)shm, nobj);
}
...
__shm_bm_ptr_free(void *ptr, size_t objsz, unsigned int nobj)
{
	...
	shm = (void *)((uword_t)ptr & ~(SHM_BM_ALIGN - 1));
