...
llboot_comp_childspdids_get(spdid_t c, u64_t *id_bits)
{
	...
	*id_bits = 0;
	ret = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_CHILDSPDIDS_GET, c, 0, &lo, &hi);
...
llboot_comp_childschedspdids_get(spdid_t c, u64_t *id_bits)
{
	...
	*id_bits = 0;
	ret = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_CHILDSCHEDSPDIDS_GET, c, 0, &lo, &hi);
	*id_bits = ((u64_t)hi << 32) | (u64_t)lo;
...
llboot_comp_init_done(void)
{
	...
	return cos_sinv(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_INIT_DONE, 0, 0);
}
...
llboot_comp_info_get(capid_t rcurfr, spdid_t spdid, pgtblcap_t *pgc, captblcap_t *capc, compcap_t *cc, spdid_t *psid, capid_t *rsfr)
{
	 ...
	 */
	r1    = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_INFO_GET, spdid, rcurfr, &r2, &r3);
	*rsfr = ((r1 << 16) >> 16);
...
llboot_comp_info_next(capid_t rcurfr, spdid_t *sid, pgtblcap_t *pgc, captblcap_t *capc, compcap_t *cc, spdid_t *psid, capid_t *rsfr)
{
	 ...
	 * for all others, it should return error!
	 */
	r1    = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_INFO_NEXT, 0, rcurfr, &r2, &r3);
	*rsfr = (int)((r1 << 16) >> 16);
...
llboot_comp_frontier_get(spdid_t spdid, vaddr_t *vasfr, capid_t *capfr)
{
	 ...
	 * for all others, it should return error!
	 */
	r1  = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, LLBOOT_COMP_FRONTIER_GET, spdid, 0, &r2, &r3);
	if (r1) return -1;
