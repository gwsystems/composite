...
again:
	for (i = 0; i < COS_THD_INIT_REGION_SIZE; i++) {
		if (__thd_init_data[cos_cpuid()][i].fn == NULL) {
			ret = cos_cas((unsigned long *)&(__thd_init_data[cos_cpuid()][i].fn), (unsigned long)NULL,
			              (unsigned long)fn);
			if (!ret) continue;

			assert(__thd_init_data[cos_cpuid()][i].fn == fn);
			__thd_init_data[cos_cpuid()][i].data = data;
...
{
	assert(idx > 0 && idx <= COS_THD_INIT_REGION_SIZE && __thd_init_data[cos_cpuid()][idx].fn);
