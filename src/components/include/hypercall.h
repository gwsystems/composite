...
static inline int 
hypercall_comp_initthd_get(spdid_t spdid, thdcap_t thdslot, arcvcap_t rcvslot, tcap_t tcslot)
{
...
static inline int
hypercall_comp_info_get(spdid_t spdid, pgtblcap_t pgtslot, captblcap_t captslot, compcap_t ccslot, spdid_t *psid)
{
	word_t r2 = 0, r3 = 0;
	int ret = 0;

	ret = cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, HYPERCALL_COMP_INFO_GET,
			     spdid << 16 | ccslot, pgtslot << 16 | captslot, &r2, &r3);
	*psid = r2;
...
static inline int
hypercall_comp_info_next(pgtblcap_t pgtslot, captblcap_t captslot, compcap_t ccslot, spdid_t *cid, spdid_t *cpid)
{
	word_t r2 = 0, r3 = 0;
	int ret = 0;

	ret =  cos_sinv_3rets(BOOT_CAPTBL_SINV_CAP, 0, HYPERCALL_COMP_INFO_NEXT,
			      ccslot, pgtslot << 16 | captslot, &r2, &r3);
	*cid  = r2;
	*cpid = r3;
