...
#define BUG() do { debug_print("BUG @ "); *volatile_null_ptr = 0; } while (0);
...
#define assert(node) do { if (unlikely(!(node))) { debug_print("FIXME: assert error in @ "); *volatile_null_ptr = 0; __cos_noret(); } } while(0)
#endif
#ifndef BUG_ON
