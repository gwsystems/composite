...
thdid_t sched_aep_create_intern(spdid_t c, int idx, int owntc, int u1, arcvcap_t *rcv, u32_t *unused);
...
sched_aep_create(struct cos_aep_info *aep, cos_aepthd_fn_t fn, void *data, int owntc)
{
	...
	ret = sched_aep_create_intern(0, idx, owntc, unused, &rcv, &unused);
	if (!ret) return 0;
