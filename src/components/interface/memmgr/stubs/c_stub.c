...
COS_CLIENT_STUB(cbuf_t, memmgr_shared_page_allocn_aligned)(struct usr_inv_cap *uc, unsigned long num_pages, unsigned long align, vaddr_t *pgaddr)
{
	word_t unused, addrret;
	cbuf_t ret;
...
COS_CLIENT_STUB(unsigned long, memmgr_shared_page_map_aligned)(struct usr_inv_cap *uc, cbuf_t id, unsigned long align, vaddr_t *pgaddr)
{
	...
	ret = cos_sinv_2rets(uc->cap_no, id, align, 0, 0, &addrret, &unused);
	*pgaddr = addrret;
