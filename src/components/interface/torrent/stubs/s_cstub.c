...
td_t __sg_tsplit(spdid_t spdid, cbuf_t cbid, int len)
{
	...
	if (unlikely(d->len[0] != 0)) return 0; 
	if (unlikely(d->len[0] >= d->len[1])) return 0;
	if (unlikely(((int)(d->len[1] + sizeof(struct __sg_tsplit_data))) != len)) return 0;
...
int __sg_tmerge(spdid_t spdid, cbuf_t cbid, int len)
{
	...
	if (unlikely(d->len[0] != 0)) return 0; 
	if (unlikely(d->len[0] >= d->len[1])) return 0;
	if (unlikely(((int)(d->len[1] + (sizeof(struct __sg_tmerge_data)))) != len)) return 0;
