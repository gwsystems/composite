...
elf32_contig_mem(struct elf_hdr *hdr, unsigned int nmem, struct elf_contig_mem *mem)
{
	...
	if (elf_chk_format(hdr) != 1 || hdr->e_phentsize != sizeof(struct elf32_proghdr)) return -1;
...
elf64_contig_mem(struct elf64_hdr *hdr, unsigned int nmem, struct elf_contig_mem *mem)
{
	...
	if (elf_chk_format((struct elf_hdr *)hdr) != 2 || hdr->e_phentsize != sizeof(struct elf64_proghdr)) return -1;
