...
cap_cons(struct captbl *t, capid_t capto, capid_t capsub, capid_t expandid)
{
	...
	if (cap_type == CAP_CAPTBL) {
		...
		/* PRY:look up the position to add this to */
		intern=pgtbl_lkup_pte(&(((struct cap_pgtbl *)ct)->pgtbl),expandid,&size_order,&old_flags);
		/* intern = pgtbl_lkup_lvl(, expandid, &flags, ct->lvl, depth); */
		...
		/* PRY:New pte's making needs to be custom */
		new_pte = COS_PGTBL_CHILD(&(((struct cap_pgtbl *)ctsub)->pgtbl),PGTBL_INTERN_DEF);
...
pgtbl_cons(struct captbl *t, capid_t capto, capid_t capsub, capid_t expandid)
{
	...
	/* PRY:do not check this now */
//	depth = ctsub->lvl;
//	if (depth == 0) return -EINVAL; /* subtree must not have a root */
	/* PRY: this assertion does not work for pgtbl. removed */
//	assert(ct->captbl);
//	if (cap_type == CAP_CAPTBL) {
//		ret = captbl_cons(ct, ctsub, expandid);
//	} else {
		...
		/* PRY:look up the position to add this to. If this is the top-level, we ignore than expandid and start direct insertion */
		if(((((struct cap_pgtbl *)ct)->pgtbl.type_addr)&COS_PGTBL_TABLE)==0)
		{
			/* Do we have a lower level? */
			intern=&(((struct cap_pgtbl *)ct)->pgtbl.type_addr);
			if((((struct cap_pgtbl *)ct)->pgtbl.type_addr)!=0)
				return -EPERM;
			old_pte = *intern;
		}
		else
		...
		if(((((struct cap_pgtbl *)ctsub)->pgtbl.type_addr)&COS_PGTBL_TABLE)==0)
			return -EPERM;
		else if(((struct cap_pgtbl *)ctsub)->pgtbl.data[9]!=0)
			return -EPERM;
		...
		{
			/* See if the top-level have a ultimate top-level. If yes, we allow the cons, or we do not allow the cons */
			if((((struct cap_pgtbl *)ct)->pgtbl.data[9])==0)
				 return -EPERM;

			if((COS_PGTBL_STARTADDR(((struct cap_pgtbl *)ct)->pgtbl.type_addr)+
			    (1<<COS_PGTBL_SIZEORDER(((struct cap_pgtbl *)ct)->pgtbl.type_addr))*expandid)>
			   COS_PGTBL_STARTADDR(((struct cap_pgtbl *)ctsub)->pgtbl.type_addr))
				return -EPERM;

			if((COS_PGTBL_STARTADDR(((struct cap_pgtbl *)ct)->pgtbl.type_addr)+
			    (1<<COS_PGTBL_SIZEORDER(((struct cap_pgtbl *)ct)->pgtbl.type_addr))*(expandid+1))<=
			   COS_PGTBL_STARTADDR(((struct cap_pgtbl *)ctsub)->pgtbl.type_addr))
				return -EPERM;
		}
		...
		if(((((struct cap_pgtbl *)ct)->pgtbl.type_addr)&COS_PGTBL_TABLE)==0)
			new_pte = COS_PGTBL_CHILD(&(((struct cap_pgtbl *)ctsub)->pgtbl),0);
		else
			new_pte = COS_PGTBL_CHILD(&(((struct cap_pgtbl *)ctsub)->pgtbl),PGTBL_INTERN_DEF);

		((struct cap_pgtbl *)ctsub)->pgtbl.data[9]=(u32_t)&(((struct cap_pgtbl *)ct)->pgtbl);
		...
		chal_MPU_update(&(((struct cap_pgtbl *)ctsub)->pgtbl),1);
//	}
...
cap_decons(struct captbl *t, capid_t cap, capid_t capsub, capid_t pruneid, unsigned long lvl)
{
	...
	if (head->type == CAP_CAPTBL) {
		...
		u32_t size_order;
		u32_t old_flags;
		intern = pgtbl_lkup_pte(&(pt->pgtbl), pruneid, &size_order, &old_flags);
	} else {
		return -EINVAL;
	}
