...
#define RETYPE_ENT_GLB_REFCNT_SZ (RETYPE_ENT_REFCNT_SZ>>1)
...
struct retype_entry_glb {
	...
	/* How many kernel pages is used as objects in this memory set? */
	u32_t kernel_ref;
} __attribute__((packed));

/* The retype table walking structure - track our path through the pges of different granularity */
struct page_record {
	int inc_cnt;
 ...
 * mapping/unmapping. */
struct retype_info {
	struct retype_entry mem_set[(N_MEM_SETS << 1)];
	char                __pad[((N_MEM_SETS_SZ << 1) % CACHE_LINE == 0) ? 0 : ((N_MEM_SETS_SZ << 1) % CACHE_LINE)];
} CACHE_ALIGNED;
...
extern struct retype_info_glb glb_retype_tbl[N_MEM_SETS << 1];
...
#define GET_RETYPE_ENTRY(idx, order) ((&(retype_tbl[get_cpuid()].mem_set[((idx) + N_MEM_SETS) >> ((order) - MIN_PAGE_ORDER)])))
#define GET_RETYPE_CPU_ENTRY(cpu, idx, order) ((&(retype_tbl[cpu].mem_set[((idx) + N_MEM_SETS) >> ((order) - MIN_PAGE_ORDER)])))
/* get the global memory set struct (used for retyping only). */
#define GET_GLB_RETYPE_ENTRY(idx, order) ((&(glb_retype_tbl[((idx) + N_MEM_SETS) >> ((order) - MIN_PAGE_ORDER)].info)))
