...
kmem_unalloc(unsigned long *pte)
{
	...
	assert(chal_chal2cos(old) & PGTBL_COSKMEM);
	retypetbl_deref((void *)(old & PGTBL_FRAME_MASK));
	*pte = chal_cos2chal(chal_chal2cos(old) & ~PGTBL_COSKMEM);
}
               ...
               unsigned long **p_pte, unsigned long *v)
{
	...
	*p_pte = pgtbl_lkup_pte(cap_pt->pgtbl, cosframe_addr, &flags);
	...
	if (!(chal_chal2cos(old_v) & PGTBL_COSKMEM)) cos_throw(err, -EINVAL);
	assert(!(chal_chal2cos(old_v) & PGTBL_QUIESCENCE));
...
kmem_deact_post(unsigned long *pte, unsigned long old_v)
{
	...
	new_v = chal_cos2chal(chal_chal2cos(old_v) & (~PGTBL_COSKMEM));
...
cap_cpy(struct captbl *t, capid_t cap_to, capid_t capin_to, capid_t cap_from, capid_t capin_from)
{
	...
	if (cap_type == CAP_CAPTBL) {
		...
		if ((chal_chal2cos(old_v) & PGTBL_COSFRAME) || !(chal_chal2cos(old_v) & PGTBL_USER)) return -EPERM;
...
cap_move(struct captbl *t, capid_t cap_to, capid_t capin_to, capid_t cap_from, capid_t capin_from)
{
	...
	if (cap_type == CAP_CAPTBL) {
		...
		cos_mem_fence();
		if ((chal_chal2cos(old_v) & PGTBL_COSFRAME) == 0) return -EPERM;
		if (chal_chal2cos(old_v_to) & (PGTBL_COSFRAME | PGTBL_PRESENT)) return -EPERM;
...
static int __attribute__((noinline)) composite_syscall_slowpath(struct pt_regs *regs, int *thd_switch)
{
	...
	case CAP_HW: {
		...
		case CAPTBL_OP_HW_MAP: {
			...
			*pte = (PGTBL_FRAME_MASK & pa) | chal_cos2chal(PGTBL_USER_DEF);
