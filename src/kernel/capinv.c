...
/* PRY:add this to avoid error */
extern struct tlb_quiescence tlb_quiescence[NUM_CPU];
/*
	       ...
	       capid_t cosframe_addr, unsigned long **p_pte, unsigned long *v)
{
	...
	*p_pte =pgtbl_lkup_pte(&(cap_pt->pgtbl), cosframe_addr, &size_order, &flags);
	...
	capid_t cap_from, capid_t capin_from)
{
	...
	if (cap_type == CAP_CAPTBL) {
		...
		f =pgtbl_lkup_pte(&(((struct cap_pgtbl *)ctfrom)->pgtbl), capin_from, &size_order, &flags);
		if (!f) return -ENOENT;
		old_v = *f;
	  ...
	  capid_t cap_from, capid_t capin_from, capid_t from_index)
{
	...
	if (cap_type == CAP_CAPTBL) {
		/* PRY: Deleted page table related contents */
	} else if (cap_type == CAP_PGTBL) {
		unsigned long *f, old_v;
		u32_t flags;
		u32_t size_order;
	...
	capid_t cap_from, capid_t capin_from)
{
	...
	if (cap_type == CAP_CAPTBL) {
		...
		moveto =pgtbl_lkup_pte(&(((struct cap_pgtbl *)ctto)->pgtbl), capin_to, &size_order, &flags);
...
composite_syscall_slowpath(struct pt_regs *regs, int *thd_switch)
{
	...
	case CAP_CAPTBL:
	{
		...
		case CAPTBL_OP_PGTBLACTIVATE:
		{
			...
			capid_t pgtbl_cap  = __userregs_get2(regs);
			u32_t info = __userregs_get3(regs);
			//vaddr_t kmem_cap   = __userregs_get3(regs);
			capid_t pgtbl_lvl  = __userregs_get4(regs);

			/* PRY: for Cortex-M, no need to allocate page table nodes, because pgtbl is deeply embedded into captbls.
			 * we just directly activate the page table. */
			//vaddr_t kmem_addr=0;
			ret = pgtbl_activate(ct, cap, pt_entry, COS_PGTBL_TYPE(info), COS_PGTBL_STARTADDR(info),
...
//			if (ret) kmem_unalloc(pte);
	...
	case CAP_PGTBL:
	{
		...
		case CAPTBL_OP_MEMDEACTIVATE:
		{
			...
			livenessid_t lid  = __userregs_get2(regs);
			/* PRY:No level checks - commented out */
		...
		case CAPTBL_OP_INTROSPECT:
		{
			...
			pte =pgtbl_lkup_pte(&(((struct cap_pgtbl *)ch)->pgtbl), addr, &size_order, &flags);
...
/* no break here? */
	...
	case CAP_HW:
	{
		...
		case CAPTBL_OP_HW_MAP:
		{
			...
			pte = pgtbl_lkup_pte(&(ptc->pgtbl), va, &size_order, &flags);
