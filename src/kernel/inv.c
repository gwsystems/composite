...
cos_syscall_tcap_cntl(int op, struct tcap *tcap1, struct tcap *tcap 2, void *arg)
{
	switch (op) {
	...
	case COS_TCAP_SPLIT:
		struct budget *b = arg;
		struct tcap *newtcap = malloc (sizeof(struct tcap));
	...
	case COS_TCAP_SETPRIORITY:
		...
		else tcap1->delegation.priority = p;
		return tcap1;
