...
kern_boot_thd(struct captbl *ct, void *thd_mem, void *tcap_mem, const cpuid_t cpu_id)
{
	...
	ret = thd_activate(ct, BOOT_CAPTBL_SELF_CT, BOOT_CAPTBL_SELF_INITTHD_BASE + captbl_per_cpu_offset(cpu_id, CAP_THD), thd_mem, BOOT_CAPTBL_SELF_COMP, 0);
	assert(!ret);

	tcap_active_init(cos_info);
	ret = tcap_activate(ct, BOOT_CAPTBL_SELF_CT, BOOT_CAPTBL_SELF_INITTCAP_BASE + captbl_per_cpu_offset(cpu_id, CAP_TCAP), tcap_mem);
	assert(!ret);
	...
	ret = arcv_activate(ct, BOOT_CAPTBL_SELF_CT, BOOT_CAPTBL_SELF_INITRCV_BASE + captbl_per_cpu_offset(cpu_id, CAP_ARCV), BOOT_CAPTBL_SELF_COMP,
	                    BOOT_CAPTBL_SELF_INITTHD_BASE + captbl_per_cpu_offset(cpu_id, CAP_THD),
			    BOOT_CAPTBL_SELF_INITTCAP_BASE + captbl_per_cpu_offset(cpu_id, CAP_TCAP), 0, 1);
	assert(!ret);
