...
getsym(bfd *obj, char *symbol)
{
	...
	if (strcmp(prev_name, service_name)) {
		...
		if (private_symtabs == NULL) { free(private_symtabs); }
	...
	for (i = 0; i < number_of_symbols; i++) {
		if (!strcmp(symbol, private_symtabs[i].name)) { return private_symtabs[i].vma_val; }
	}
...
make_cobj_symbols(struct service_symbs *s, struct cobj_header *h)
{
	...
	for (i = 0; i < s->exported.num_symbs; i++) {
		...
		/* ST_user_caps offset is 0 when not relevant. */
		if (cobj_symb_init(h, symb_offset++, s->exported.symbs[i].name, COBJ_SYMB_EXPORTED,
		                   s->exported.symbs[i].addr, 0)) {
...
genscript(int with_addr, char *tmp_exec, char *script)
{
	...
	for (i = 0; section_info[i].secid != MAXSEC_S; i++) {
		if (with_addr && !section_info[i].coalesce) { emit_address(fp, section_info[i].start_addr); }
