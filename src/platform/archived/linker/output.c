...
int
is_hl_booter_loaded(struct service_symbs *s)
{
                      ...
                      char *server_fn, int flags)
{
	...
	for (i = 0; i < st->num_symbs; i++) {
		if (strcmp(client_fn, st->symbs[i].name) == 0) { break; }
	}
...
spd_contains_symb(struct service_symbs *s, char *name)
{
	...
	for (i = 0; i < symbs->num_symbs; i++) {
		...
		if (strcmp(name, n) == 0) { return &symbs->symbs[i]; }
...
cap_get_info(struct service_symbs *service, struct cap_ret_info *cri, struct symb *symb)
{
	...
	if (MAX_SYMB_LEN - 1
	    == snprintf(tmp, MAX_SYMB_LEN - 1, "%s%s", exp_symb->name, CAP_SERVER_STUB_POSTPEND_RETS)) {
	...
	if (NULL == s_stub) {
		...
		if (MAX_SYMB_LEN - 1
		    == snprintf(tmp, MAX_SYMB_LEN - 1, "%s%s", exp_symb->name, CAP_SERVER_STUB_POSTPEND)) {
			printl(PRINT_HIGH, "symbol name %s too long to become server capability\n", exp_symb->name);
			return -1;
		}
		...
		if (NULL == s_stub) {
			printl(PRINT_HIGH,
			       ...
			       exp_symb->name, exporter->obj, service->obj);
			return -1;
		}
	} else {
		is_rets = 1;
	...
	if (exp_symb->modifier_offset) {
		printf("%d: %s\n", service_get_spdid(exporter), exp_symb->name + exp_symb->modifier_offset);
	}
...
make_cobj_caps(struct service_symbs *s, struct cobj_header *h)
{
	...
	for (i = 0; i < undef_symbs->num_symbs; i++) {
		 ...
		 * In order to get the actual symbol index of the current undefined symbol, we need to undo that
		 * operation.
		 */
...
void
make_spd_boot_schedule(struct service_symbs *comp, struct service_symbs **sched, unsigned int *off)
{
	int i;
	...
	for (i = 0; i < comp->num_dependencies; i++) {
		...
		if (!spd_already_loaded(d->dep)) { make_spd_boot_schedule(d->dep, sched, off); }
...
make_spd_boot(struct service_symbs *boot, struct service_symbs *all)
{
	...
	for (all = first; NULL != all; all = all->next) { make_spd_boot_schedule(all, schedule, &off); }
	...
	assert((u32_t)(new_end - new_sect_start) + 3 * PAGE_SIZE == cobj_sect_get(new_h, INITFILE_S)->bytes);
	...
	for (i = 0; i < off; i++) { ((int *)new_end)[i] = service_get_spdid(schedule[i]); }
	((int *)new_end)[off] = 0;
...
make_spd_llboot(struct service_symbs *boot, struct service_symbs *all)
{
	...
	for (all = first; NULL != all; all = all->next) {
		...
		*heap_ptr = (void *)(((int)heap_ptr_val) + obj_size);
		...
		if (write(co_fd, h, h->size) == -1) { printl(PRINT_HIGH, "Error outputting co.\n"); }
		close(co_fd);
	}
...
{
	while (s) {
		if (!strncmp(&s->obj[5], n, strlen(n))) { return s; }
...
output_image(struct service_symbs *services)
{
	...
	if ((s = find_obj_by_name(services, BOOT_COMP))) { make_spd_boot(s, services); }
