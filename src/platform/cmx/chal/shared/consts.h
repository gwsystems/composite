...
/*
 * PRY: the page size is always 1KB when we are setting up the booter component
 * Register definition moved to other places. This will be used as a standard
 * page size.
 */
#define PAGE_ORDER 12
#define MIN_PAGE_ORDER 8
#ifndef __KERNEL__
#define PAGE_SIZE (1<<PAGE_ORDER)
#define MIN_PAGE_SIZE (1<<MIN_PAGE_ORDER)
#endif

#define MAX_SERVICE_DEPTH 31
#define MAX_NUM_THREADS   8

#define STACK_PAGE_SZ   (1<<9)
/* Stacks are 2 * page_size (expressed in words)  */
#define MAX_STACK_SZ    (STACK_PAGE_SZ/4) /* (PAGE_SIZE/4) */ /* a page */
...
/* # of pages */
#define COS_MAX_MEMORY    (COS_MEM_KERN_PA_SZ/MIN_PAGE_SIZE)  /* # of pages */
#define COS_MEM_BOUND     (COS_MEM_KERN_PA + COS_MAX_MEMORY*MIN_PAGE_SIZE) /* highest physical address */

/* These are deprecated, use the macros they reference */
#define KERN_MEM_ORDER    (COS_MEM_KERN_PA_ORDER-MIN_PAGE_ORDER)
#define COS_KERNEL_MEMORY (COS_MEM_KERN_PA_SZ/MIN_PAGE_SIZE)   /* 2^n pages kernel memory */
...
#define RETYPE_MEM_SIZE          (RETYPE_MEM_NPAGES * MIN_PAGE_SIZE)

#include "../asm_ipc_defs.h" 	/* FIXME: just for cos_component.h now */
/* PRY:This is not used anyway */
#define KERN_BASE_ADDR COS_MEM_KERN_START_VA
