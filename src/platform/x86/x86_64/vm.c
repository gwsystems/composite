...
u64_t boot_ap_pgd[PAGE_SIZE / sizeof(u64_t)] PAGE_ALIGNED = {[0] = 0 | X86_PGTBL_PRESENT | X86_PGTBL_WRITABLE
                                                                   | X86_PGTBL_SUPER,
...
kern_setup_image(void)
{
	...
	for (i = kern_pa_start; i < (unsigned long)round_up_to_pgt3_page(kern_pa_end); i += PGT3_RANGE, j++) {
		boot_comp_pgt3[i / PGT3_RANGE] = i | X86_PGTBL_PRESENT | X86_PGTBL_WRITABLE | X86_PGTBL_SUPER
		                                 | X86_PGTBL_GLOBAL;
	}
...
device_map_mem(paddr_t dev_addr, unsigned int pt_extra_flags)
{
	...
	if (vaddr) {
		boot_comp_pgt3[((unsigned long)vaddr - (unsigned long)(COS_MEM_KERN_START_VA)) / PGT3_RANGE] |=
		  pt_extra_flags; /* use the union of the flags */
	...
	boot_comp_pgt3[off] = rounded | X86_PGTBL_PRESENT | X86_PGTBL_WRITABLE | X86_PGTBL_SUPER | X86_PGTBL_GLOBAL
	                      | pt_extra_flags;
	dev_mem[dev_map_off] = (struct dev_map){.physaddr = rounded,
