...
int make_cobj_symbols(struct service_symbs *s, struct cobj_header *h)
{
        u32_t addr;
        u32_t symb_offset = 0;
        int i;
...
int calc_offset(int offset, asection *sect)
{
        int align;
...
int calculate_mem_size(int first, int last)
{
        int offset = 0;
        int i;
...
void emit_address(FILE *fp, unsigned long addr)
{
        fprintf(fp, ". = 0x%x;\n", (unsigned int)addr);
}
...
void findsections_ldobj(bfd *abfd, asection *sect, PTR obj)
{
...
void emit_section(FILE *fp, char *sec)
{
        /*
...
int genscript(int with_addr, char *tmp_exec, char *script)
{
        FILE *fp;
