...
int
create_spd_capabilities(struct service_symbs *service/*, struct spd_info *si*/, int cntl_fd)
{
	int i;
...
struct spd_info *
create_spd(int cos_fd, struct service_symbs *s, long lowest_addr, long size) 
{
	struct spd_info *spd;
...
void
make_spd_scheduler(int cntl_fd, struct service_symbs *s, struct service_symbs *p)
{
	vaddr_t sched_page;
...
struct
service_symbs *find_obj_by_name(struct service_symbs *s, const char *n)
{
	while (s) {
...
output_image(struct service_symbs *services)
{
	...
	fn = (int *)get_symb_address(&s->exported, "spd0_main");

	assert(0 == cos_create_thd(cntl_fd, &thd));
