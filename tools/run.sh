...
if [ "${arch}" == "x86_64" ]
then
	qemu-system-x86_64 ${kvm_flag} -cpu host -smp ${vcpus},cores=${num_cores},threads=${num_threads},sockets=${num_sockets} -m ${mem_size} -cdrom $1 -no-reboot -nographic -s ${debug_flag} -nic none ${nic_flag}
elif [ "${arch}" == "i386" ]
then
